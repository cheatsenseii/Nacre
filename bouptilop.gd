...
func _ready() -> void:
	...
	var cream:=Color("b9aa82");var dark:=Color("201b1b")
	...
	var hat: MeshInstance3D=game.combat.shape(cap,Vector3.ZERO,Vector3(0.96,0.36,0.84),Color("7f3028"),true)
	hat.material_override.roughness=0.46
	game.combat.shape(cap,Vector3(0,-0.09,0),Vector3(0.85,0.05,0.73),Color("c5b489"),true)
	for i in range(9):
		var a:=i*2.39996;var radius:=0.12+0.21*sqrt(float(i)/9)
		game.combat.shape(cap,Vector3(cos(a)*radius,0.17-radius*0.13,sin(a)*radius*0.8),Vector3(0.10,0.035,0.08),Color("d2c49a"),true)
	for i in range(2):
		var side: float=-1 if i==0 else 1
		var eye: MeshInstance3D=game.combat.shape(model,Vector3(side*0.105,0.66+i*0.035,-0.163),Vector3(0.13,0.16,0.085),Color("e7d9ad"),true)
	...
	game.combat.shape(model,Vector3(0,0.415,-0.204),Vector3(0.10,0.065,0.025),Color("765052"),true)
